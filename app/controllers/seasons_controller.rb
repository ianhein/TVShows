class SeasonsController < ApplicationController
  ...
  def index
    @seasons = Season.all
    @seasons = Season.where(:series_id => params[:series_id]) if params[:series_id]
    #Return nombre de la season
    @seasons = Season.where(:name => params[:name]) unless params[:name].blank?
  end
